...
export class TransferenciaService {
  ...
  transferir(transferenciaDTO: TransferenciaDTO): Observable<any> {
    ...
    return this.httpRequest
      .put(`${uri}transferencia/transferir/${contaID}`, transferenciaDTO, {
  ...
  buscarBeneficiario(numeroContaDestino: string): Observable<any> {
    return this.httpRequest
      .get(`${uri}Contabancaria/beneficiario/${numeroContaDestino}`, {
        headers: new HttpHeaders({
          authorization: `Bearer ${this.usuarioLogged.webToken.token}`,
        }),
      })
